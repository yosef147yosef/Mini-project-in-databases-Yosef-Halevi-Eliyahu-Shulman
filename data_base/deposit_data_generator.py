...
def main():
    # Set the number of deposits to generate
    num_deposits = 5000  # Change this value to generate a different number of deposits

    # Read existing data or generate if necessary
    bank_ids = read_bank_ids('bank.csv')
    used_deposit_ids = set()
    used_interest_rate_ids = set()
    used_account_numbers = [generate_random_account_number() for _ in range(1000)]  # Assuming 1000 existing accounts
    used_worker_ids = [str(random.randint(1000, 9999)) for _ in range(100)]  # Assuming 100 existing workers

    # Generate the specified number of random deposits
    deposits = [generate_random_deposit(bank_ids, used_deposit_ids, used_interest_rate_ids, used_account_numbers, used_worker_ids) for _ in range(num_deposits)]
